...
$imageUrl = (strpos($img, 'data:image/') === 0) ? save_data_url_as_upload($img, $id) : trim($img);
if ($imageUrl === '') {
    $stImg = $pdo->prepare('SELECT image_url FROM content_items WHERE id = ?');
    $stImg->execute([$id]);
    $imageUrl = (string) ($stImg->fetchColumn() ?: '../images/book.png');
}
...
try {
    ...
    $updated = $pdo->prepare('UPDATE content_items SET title=?, description=?, image_url=?, category_id=?, year=?, length=?, age_rating=? WHERE id=?')
        ->execute([$title, $desc, $imageUrl, $categoryId, $year, $length, $age, $id]);
    if (!$updated) {
        throw new RuntimeException('update_failed');
    }

    $pdo->prepare('DELETE FROM content_creators WHERE item_id = ?')->execute([$id]);
    ...
    $pdo->prepare('INSERT INTO content_creators (item_id, creator_id) VALUES (?,?)')->execute([$id, $creatorId]);

    $pdo->prepare('DELETE FROM content_genres WHERE item_id = ?')->execute([$id]);
    if ($extraTags) {
        ...
        foreach ($extraTags as $tagName) {
            ...
            $stmtInsertGenre->execute([$id, $genreId]);
        }
    }
    ...
    error_log('Cultify item_update error: ' . $e->getMessage());
